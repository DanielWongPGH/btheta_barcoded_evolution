...
parser.add_argument('--noise-estimate-dir', type=Path)
parser.add_argument('--output-dir', type=Path, default=Path('.'))
parser.add_argument('--relabels', type=Path, default=Path(__file__).with_name('library_mislabelings.tsv'))
args = parser.parse_args()
data_dir = args.data_dir
noise_estimate_dir = args.noise_estimate_dir or data_dir / 'process_barseq' / 'pseudo_derep'
...
for experiment, directory, files in zip(['E1', 'E2', 'vitro'],
    ...
    for file in sorted(files):
        ...
        ## get effective depths for each library
        ## 
        try:
            with open(f'{noise_estimate_dir}/{file.replace("cluster.csv", "noise.out")}', 'r') as f_derep:
                header = next(f_derep)
                noise_dict = {}
                for line in f_derep:
