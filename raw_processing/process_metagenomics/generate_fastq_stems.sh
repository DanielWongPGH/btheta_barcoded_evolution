#!/bin/bash
# Set metagenomic manifest headers from project_config.yaml without changing samples.
...
if (( $# > 0 )); then
    ...
    manifests=("${SCRIPT_DIR}"/*fastq_stems*.txt)
fi
...
for manifest in "${manifests[@]}"; do
    ...
    fastq_dir="$(yq -r ".raw_processing.metagenomics.manifest_fastq_dirs.\"${key}\" // \"\"" "$config_file")"
    fastq_dir="${fastq_dir%/}"

    if [[ -z "$fastq_dir" || "$fastq_dir" == "null" || "$fastq_dir" == /path/to/* ]]; then
        echo "Error: configure raw_processing.metagenomics.manifest_fastq_dirs.${key} in $config_file" >&2
        exit 1
    fi
