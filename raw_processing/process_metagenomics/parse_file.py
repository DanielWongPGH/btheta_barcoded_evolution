...
default_chromosome_name = "NC_004663" # <-- change this to the new one
# default_reference_filename = "BtVPI-5482.gbk"
default_reference_filename = "MG1655_sequence.gb" # <-- change this to the new one, but new one doesnt have repeats
#default_reference_filename = "REL606.6.gbk"
...
def parse_reference_genome(filename=default_reference_filename):
    reference_sequences = []
    
    # GBK file
    filename_stem = (filename.strip()).split(".")[-1]
    if filename_stem.startswith('gb'):
        file = open(filename,"r")
...
#####
#
# Helper function for parsing genbank files
#
####
def get_feature(line):
    #print "getting feature from:", line.strip()
    ...
    items = line.split()
    potential_feature = items[0].strip()
    
    if len(items)>1:
        potential_location=items[1].strip()
    else:
        potential_location=""
    ...
    if ".." in potential_location:
        ...
        if potential_feature in features:
...
#
#####################################################################        
def parse_gene_list(reference_sequence=None, filename=default_reference_filename):
    ...
    while line!="":
        ...
        if feature=='CDS':
            ...
            while get_feature(line)=="":
                
                if line.strip().startswith('ORIGIN'):
                    print "ORIGIN!"
                #print "Not a feature:", line
...
def parse_repeat_list(filename=default_reference_filename):
    ...
    while line!="":
        ...
        if feature == 'repeat_region':
            ...
            line = file.readline()
            stripped_line = line.strip()
            while (stripped_line[0]=='/') or (stripped_line==""):
                if line.strip().startswith('/mobile_element'):
                    repeat_name = line.split('=')[1].strip()[1:-1]
                
                if repeat_name=='unknown' and stripped_line.startswith('/note="'):
                    repeat_name = ("repeat element:%s" % stripped_line[7:].split()[0] ) 
                line = file.readline()
                stripped_line = line.strip()
