...
for (sample_dir, sample_name), sample_time in zip(population_parameters.sample_names[population], population_parameters.sample_times[population]):
    if sample_name in sample_names:
        sample_list.append(sample_name)
        sample_times.append(int(sample_time))

# make sure the list matches the order of samples passed on command line
# (which matches the mpileup column order)
# sample_order = {name: i for i, name in enumerate(sample_names)}
# sample_list, sample_times = zip(*sorted(zip(sample_list, sample_times), key=lambda x: sample_order[x[0]]))
...
for line in mpileup_file:
    ...
    for allele_key in merged_alts.keys():
        ...
        # more lenient for lower read count (i.e. MGX)
        if ((allele_alts>=1).sum() > 1) and ((allele_alts >= 1)*(allele_depths>=3)*(allele_alts >= 0.05*allele_depths)).sum() > 0:
            print ", ".join([chromosome, str(position), alt_allele, " ".join(str(t) for t in merged_times), " ".join(str(a) for a in allele_alts), " ".join(str(d) for d in allele_depths)])
            num_printed_lines+=1
