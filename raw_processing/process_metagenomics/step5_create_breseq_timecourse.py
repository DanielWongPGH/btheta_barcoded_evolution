...
from parse_file import parse_repeat_list, get_repeat_idx, get_closest_repeat_idx, default_chromosome_name
...
full_sample_list = [sample for (directory, sample) in population_parameters.sample_names[population]]
...
for gd_filename in gd_filenames:
    #sys.stdout.write(gd_filename)
   # sample_name = (gd_filename.split("rebreseq_output/{}/".format(population))[1]).split("/")[0].strip() 
    sample_name = gd_filename.replace("/output/evidence/evidence.gd", "").split("/")[-1]
...
for t,sample_name in zip(sample_times, sample_list):
    ...
    for short_mutation_string in renormalized_mutation_map.keys(): 
        ...
        for mutation_string in renormalized_mutation_map[short_mutation_string].keys():
            ...
            mutation_map[t][mutation_string] = ((default_chromosome_name, location, mutation_string),  alt, depth)
    ...
    for indel_string in merged_indel_map.keys(): 
        ...
        if indel_string in mutation_map[t]:
            ...
            A = old_A + A
            D = old_D + D
        mutation_map[t][indel_string] = ((default_chromosome_name, location, indel_string),  A, D)
